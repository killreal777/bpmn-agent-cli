...
function addedElements(base, candidate) {
  return sortedElements([...candidate.byId.values()].filter((element) => !base.byId.has(element.id)));
}
function removedElements(base, candidate) {
  return sortedElements([...base.byId.values()].filter((element) => !candidate.byId.has(element.id)));
}
function renamedElements(base, candidate) {
  return commonElementIds(base, candidate).map((id) => ({
    before: base.byId.get(id),
    after: candidate.byId.get(id)
  })).filter((change) => Boolean(change.before && change.after)).filter((change) => change.before.name !== change.after.name).sort((a, b) => a.before.id.localeCompare(b.before.id));
}
function reconnectedFlows(base, candidate) {
  return [...base.sequenceFlowById.keys()].filter((id) => candidate.sequenceFlowById.has(id)).map((id) => ({
    id,
    before: base.sequenceFlowById.get(id),
    after: candidate.sequenceFlowById.get(id)
  })).filter((change) => Boolean(change.before && change.after)).filter((change) => change.before.sourceId !== change.after.sourceId || change.before.targetId !== change.after.targetId || change.before.condition !== change.after.condition).sort((a, b) => a.id.localeCompare(b.id));
}
function changedImplementations(base, candidate) {
  return commonElementIds(base, candidate).map((id) => {
  ...
  }).filter((change) => Boolean(change.element)).filter((change) => canonicalJson(change.before) !== canonicalJson(change.after)).sort((a, b) => a.element.id.localeCompare(b.element.id));
}
function changedDocumentation(base, candidate) {
  return commonElementIds(base, candidate).map((id) => ({
    element: candidate.byId.get(id) ?? base.byId.get(id),
    before: documentationText(base.rawById.get(id)),
    after: documentationText(candidate.rawById.get(id))
  })).filter((change) => Boolean(change.element)).filter((change) => change.before !== change.after).sort((a, b) => a.element.id.localeCompare(b.element.id));
}
function commonElementIds(base, candidate) {
  return [...base.byId.keys()].filter((id) => candidate.byId.has(id)).sort((a, b) => a.localeCompare(b));
}
function documentationText(element) {
  const values = arrayOf2(element?.documentation).map((documentation) => stringValue(documentation.text) ?? stringValue(documentation.body)).filter((value) => Boolean(value));
  return values.length > 0 ? values.join("\n\n") : null;
}
function sortedElements(elements) {
  return [...elements].sort((a, b) => a.id.localeCompare(b.id));
}
function sortedImplementations(implementations) {
...
// src/cli/commands/diffCommand.ts
